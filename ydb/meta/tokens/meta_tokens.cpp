...
#include "meta_tokens.h"

namespace NMeta {

TMetaTokenator* TMetaTokenator::CreateTokenator(const TTokensConfig& tokensConfig, TGrpc& grpc, const NActors::TActorId& httpProxy) {
    return new TMetaTokenator(tokensConfig, grpc, httpProxy);
}

TString TMetaTokenator::GetToken(const TString& name) {
    TString token;
    {
        ...
        if (it != Tokens.end()) {
            ...
            BLOG_ERROR("Requested token " << name << " not found");
        }
    }
    return token;
}

TMetaTokenator::TMetaTokenator(TTokensConfig tokensConfig, TGrpc& grpc, const NActors::TActorId& httpProxy)
    : Grpc(grpc)
    , HttpProxy(httpProxy)
{
    ...
    for (const TJwtInfo& jwtInfo : tokensConfig.jwtinfo()) {
        TokenConfigs.JwtTokenConfigs[jwtInfo.name()] = jwtInfo;
    }
    for (const TOAuthInfo& oauthInfo : tokensConfig.oauthinfo()) {
        TokenConfigs.OauthTokenConfigs[oauthInfo.name()] = oauthInfo;
    }
    for (const TMetadataTokenInfo& metadataTokenInfo : tokensConfig.metadatatokeninfo()) {
        TokenConfigs.MetadataTokenConfigs[metadataTokenInfo.name()] = metadataTokenInfo;
    }
    for (const TStaticCredentialsInfo& staticCredentialsInfo : tokensConfig.staticcredentialsinfo()) {
        TokenConfigs.StaticCredentialsConfigs[staticCredentialsInfo.name()] = staticCredentialsInfo;
    }
    for (const TStaticTokenInfo& staticTokenInfo : tokensConfig.statictokeninfo()) {
        TokenConfigs.StaticTokensConfigs[staticTokenInfo.name()] = staticTokenInfo;
    }
    TokenConfigs.AccessServiceType = tokensConfig.accessservicetype();
}

void TMetaTokenator::Bootstrap() {
    ...
    for (const auto& [name, config] : TokenConfigs.StaticTokensConfigs) {
        Tokens[name] = config.token();
    }
    Become(&TMetaTokenator::StateWork, PERIODIC_CHECK, new NActors::TEvents::TEvWakeup());
}

void TMetaTokenator::HandlePeriodic() {
...
void TMetaTokenator::Handle(TEvPrivate::TEvRefreshToken::TPtr event) {
    TString name = event->Get()->Name;
    BLOG_D("Refreshing token " << name);
    const TJwtInfo* jwtInfo = TokenConfigs.GetJwtTokenConfig(name);
    if (jwtInfo != nullptr) {
        UpdateJwtToken(jwtInfo);
        return;
    }
    const TOAuthInfo* oauthInfo = TokenConfigs.GetOAuthTokenConfig(name);
    if (oauthInfo != nullptr) {
        UpdateOAuthToken(oauthInfo);
        return;
    }
    const TMetadataTokenInfo* metadataTokenInfo = TokenConfigs.GetMetadataTokenConfig(name);
    if (metadataTokenInfo != nullptr) {
        UpdateMetadataToken(metadataTokenInfo);
        return;
    }
    const TStaticCredentialsInfo* staticCredentialsInfo = TokenConfigs.GetStaticCredentialsTokenConfig(name);
...
void TMetaTokenator::Handle(TEvPrivate::TEvUpdateIamTokenYandex::TPtr event) {
...
void TMetaTokenator::Handle(TEvPrivate::TEvUpdateIamTokenNebius::TPtr event) {
    TDuration refreshPeriod = SUCCESS_REFRESH_PERIOD;
    if (event->Get()->Status.Ok()) {
...
void TMetaTokenator::Handle(TEvPrivate::TEvUpdateStaticCredentialsToken::TPtr event) {
    TDuration refreshPeriod = SUCCESS_REFRESH_PERIOD;
...
const TJwtInfo* TMetaTokenator::TTokenConfigs::GetJwtTokenConfig(const TString& name) {
    return GetTokenConfig(JwtTokenConfigs, name);
}

const TOAuthInfo* TMetaTokenator::TTokenConfigs::GetOAuthTokenConfig(const TString& name) {
    return GetTokenConfig(OauthTokenConfigs, name);
}

const TMetadataTokenInfo* TMetaTokenator::TTokenConfigs::GetMetadataTokenConfig(const TString& name) {
    return GetTokenConfig(MetadataTokenConfigs, name);
}

const TStaticCredentialsInfo* TMetaTokenator::TTokenConfigs::GetStaticCredentialsTokenConfig(const TString& name) {
    return GetTokenConfig(StaticCredentialsConfigs, name);
}

const TStaticTokenInfo* TMetaTokenator::TTokenConfigs::GetStaticTokenConfig(const TString& name) {
    return GetTokenConfig(StaticTokensConfigs, name);
}

void TMetaTokenator::UpdateStaffApiUserToken(const TStaffApiUserTokenInfo* staffApiUserTokenInfo) {
    Tokens[staffApiUserTokenInfo->name()] = "OAuth " + staffApiUserTokenInfo->token();
}

void TMetaTokenator::UpdateStaticCredentialsToken(const TStaticCredentialsInfo* staticCredentialsInfo) {
...
void TMetaTokenator::UpdateMetadataToken(const TMetadataTokenInfo* metadataTokenInfo) {
...
void TMetaTokenator::Handle(NHttp::TEvHttpProxy::TEvHttpIncomingResponse::TPtr event) {
...
void TMetaTokenator::UpdateJwtToken(const TJwtInfo* jwtInfo) {
...
void TMetaTokenator::UpdateOAuthToken(const TOAuthInfo* oauthInfo) {
    yandex::cloud::priv::iam::v1::CreateIamTokenRequest request;
...
template<>
TString SecureShortDebugString(const Ydb::Auth::LoginRequest& request) {
    Ydb::Auth::LoginRequest copy = request;
    copy.set_password("****");
    return copy.ShortDebugString();
}

...
class THandlerActorMetaCreateDatabaseRequest : public THandlerActorMetaRequest<THandlerActorMetaCreateDatabaseRequest> {
public:
    using TBase = THandlerActorMetaRequest<THandlerActorMetaCreateDatabaseRequest>;
    ...
    void OnClusterReady(const NJson::TJsonValue& cluster) override {
        auto controlPlaneEndpoint = GetControlPlaneEndpoint(cluster);

    }

    void CallControlPlane() {
        NYdbGrpc::TResponseCallback<ydb::yc::priv::operation::Operation> responseCb =
            [actorId = SelfId()](NYdbGrpc::TGrpcStatus&& status, ydb::yc::priv::operation::Operation&& response) -> void {
                if (auto ydbMeta = InstanceYdbMeta.lock()) {
                    if (status.Ok()) {
                        ydbMeta->ActorSystem->Send(actorId, new TEvPrivate::TEvOperationResponse(std::move(response)));
                    } else {
        ...
        if (!ParsePostData(controlPlaneRequest)) {
            return;
        }
        ...
        connection->DoRequest(ControlPlaneRequest, std::move(responseCb), &yandex::cloud::priv::ydb::v1::DatabaseService::Stub::AsyncCreate, meta);
    }
    ...
    void Handle(TEvPrivate::TEvExecuteQueryResult::TPtr event) {
        ...
        if (result.IsSuccess()) {
            ...
            if (rsParser.TryNextRow()) {
                rsParser.GetValue("control_plane");
                ...
                response = Request.Request->CreateResponseBadRequest("Cluster not found", "text/plain");
            }
        } else {
    ...
    void Handle(TEvPrivate::TEvOperationResponse::TPtr event) {
        TStringStream stream;
        NJson::TJsonValue json;
        NProtobufJson::Proto2Json(event->Get()->Operation, json, Proto2JsonConfig);
        yandex::cloud::priv::ydb::v1::CreateDatabaseMetadata meta;
        if (event->Get()->Operation.metadata().UnpackTo(&meta)) {
            NProtobufJson::Proto2Json(meta, json["metadata"], Proto2JsonConfig);
        }
        auto response = Request.Request->CreateResponseOK(stream.Str(), "application/json; charset=utf-8");
        Send(Request.Sender, new NHttp::TEvHttpProxy::TEvHttpOutgoingResponse(response));
        PassAway();
    }

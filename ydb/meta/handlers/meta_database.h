...
class THandlerActorMetaCreateDatabaseRequest : THandlerActorYdb, public NActors::TActorBootstrapped<THandlerActorMetaCreateDatabaseRequest> {
public:
    using TBase = NActors::TActorBootstrapped<THandlerActorMetaCreateDatabaseRequest>;
    NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr Event;
    TRequest Request;

    THandlerActorMetaCreateDatabaseRequest(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event)
        : TBase(&THandlerActorMetaCreateDatabaseRequest::StateWork)
...
public:
    ...
    void Handle(NHttp::TEvHttpProxy::TEvHttpIncomingRequest::TPtr event) {
        ...
        if (request->Method == "POST" && contentType == "application/json") {
            Register(new THandlerActorMetaCreateDatabaseRequest(std::move(event)));
            return;
        }

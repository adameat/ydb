...
public:
    ...
    void ReplyWithBadRequest(const NActors::TActorContext& ctx, const TString& errorMsg) {
        ctx.Send(Request.Sender, new NHttp::TEvHttpProxy::TEvHttpOutgoingResponse(Request.Request->CreateResponseBadRequest(errorMsg, "text/plain")));
        TBase::Die(ctx);
    }
    ...
    void Bootstrap(const NActors::TActorContext& ctx) {
        ...
        if (Request.Parameters.PostData.IsDefined()) {
            try {
                ...
                ReplyWithBadRequest(ctx, e.what());
                return;
            }
        }

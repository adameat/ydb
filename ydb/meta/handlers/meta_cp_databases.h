...
public:
    ...
    void SendControlPlaneRequest() {
        ...
        CrackEndpoint(ControlPlaneEndpoint, scheme, tokenName, host, uri);
        TString endpoint = TString(scheme) + "://" + host;
        TMetaTokenator* tokenator = MetaAppData()->Tokenator;
        if (tokenator && tokenName) {
            TString token = tokenator->GetToken(TString(tokenName));
            if (token) {
                Request.SetHeader(meta, "authorization", token);
            }
        }
        meta.Timeout = GetClientTimeout();
        auto connection = YdbMeta->Grpc.CreateGRpcServiceConnectionFromEndpoint<yandex::cloud::priv::ydb::v1::DatabaseService>(endpoint);
    ...
    void Handle(TEvPrivate::TEvExecuteQueryResult::TPtr event) {
        ...
        if (result.IsSuccess()) {
            ...
            if (rsParser.TryNextRow()) {
                ...
                ControlPlaneEndpoint = ColumnValueToString(rsParser.GetValue("control_plane"));
                TString controlPlaneTokenName = ColumnValueToString(rsParser.GetValue("mvp_token"));
                TStringBuf scheme = "grpc";
                TStringBuf host;
                TStringBuf uri;
                TStringBuf tokenName;
                CrackEndpoint(ControlPlaneEndpoint, scheme, tokenName, host, uri);
                if (!tokenName && controlPlaneTokenName) {
                    tokenName = controlPlaneTokenName;
                }
                ControlPlaneEndpoint = TString(scheme) + "://" + (tokenName ? TString(tokenName) + "@" : "") + host;
    ...
    void Handle(TEvPrivate::TEvErrorResponse::TPtr event) {
        if (event->Get()->Status.StartsWith("4") || DatabaseRequestDeadline <= NActors::TActivationContext::Now()) {

...
#include <ydb/public/api/client/yc_private/ydb/v1/console_service.pb.h>
...
struct THandlerActorYdb {
    ...
    /* static void WriteSchemeEntryPermissions(NJson::TJsonValue& value,
        ...
        for (const NYdb::NScheme::TPermissions& permission : permissions) {
            ...
            for (const auto& perm : permission.PermissionNames) {
                subjectPermissions.emplace(TString{perm});
            }
        }
    ...
    static void WriteColumns(NJson::TJsonValue& columns,
                             ...
                             const std::vector<std::string>& columnsKeysMeta = std::vector<std::string>(),
                             const std::function<void(NJson::TJsonValue&, NYdb::TType)>& columnTypeFormatter = ColumnTypeToString) {
    ...
    static void WriteIndexes(NJson::TJsonValue& indexes,
                             const std::vector<NYdb::NTable::TIndexDescription>& indexesMeta) {
        ...
        for (const NYdb::NTable::TIndexDescription& indexMeta : indexesMeta) {
            ...
            for (const auto& column : indexMeta.GetIndexColumns()) {
                indexColumns.AppendValue(TString{column});
            }
            ...
            for (const auto& column : indexMeta.GetDataColumns()) {
                dataColumns.AppendValue(TString{column});
            }
        }
    }
    ...
    static void WriteShards(NJson::TJsonValue& shards,
        ...
        for (ui64 nPart = 0; nPart < tableDescription.GetPartitionsCount(); ++nPart) {
            ...
            if (nPart > 0 && nPart < ranges.size()) {
                ...
                NYdb::TValueParser parser(range.From()->GetValue());
    ...
    static void WriteTopicConsumers(NJson::TJsonValue& result, const std::vector<NYdb::NTopic::TConsumer>& consumers) {
    ...
    static void WriteTopicPartitions(NJson::TJsonValue& result, const std::vector<NYdb::NTopic::TPartitionInfo>& partitionsInfo) {
        result.SetType(NJson::JSON_ARRAY);
...
class THandlerActorMetaRequest : public THandlerActorYdb, public NActors::TActorBootstrapped<TDerivedType> {
public:
    using TBase = NActors::TActorBootstrapped<TDerivedType>;
    ...
    void ResolveClusterName() {
        if (Request.Parameters.PostData.IsDefined()) {
            ...
            return ReplyBadRequest("Invalid post data");
        }
    }

    template<typename TMessage>
    bool ParsePostData(TMessage& message) {
        try {
    ...
    void ReplyBadRequest(const TString& error) {
        ...
        TBase::Send(Request.Sender, new NHttp::TEvHttpProxy::TEvHttpOutgoingResponse(response));
        TBase::PassAway();
    }
    ...
    virtual void OnClusterReady(const NJson::TJsonValue& cluster) {
        ...
        if (result.IsSuccess()) {
            if (!result.GetResultSets().empty()) {
                ...
                if (rsParser.TryNextRow()) {
                    OnClusterReady(RowToJsonValue(resultSet, rsParser));
                } else {

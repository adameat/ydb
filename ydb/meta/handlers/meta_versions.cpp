...
public:
    ...
    TVersionListLoadActor(const NActors::TActorId& owner, std::shared_ptr<TYdbMeta> ydbMeta)
        : Owner(owner)
        , YdbMeta(ydbMeta)
    {}
...
NActors::TActorId CreateLoadVersionsActor(const NActors::TActorId& owner, std::shared_ptr<TYdbMeta> ydbMeta, const NActors::TActorContext& ctx) {
    return ctx.Register(new TVersionListLoadActor(owner, ydbMeta));
}

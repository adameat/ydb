...
void TYdbMeta::AddSession(const TString& sessionId, const TActorId& actorId) {
    BLOG_D("Add session " << sessionId);
    std::lock_guard guard(SessionsMutex);
    Sessions[sessionId] = actorId;
}

void TYdbMeta::EndSession(const TString& sessionId, const TActorId& actorId) {
    BLOG_D("Ended session " << sessionId);
    std::lock_guard guard(SessionsMutex);
    auto it = Sessions.find(sessionId);
    if (it != Sessions.end() && it->second == actorId) {
        Sessions.erase(it);
    }
}

TActorId TYdbMeta::FindSession(const TString& sessionId) {
    std::lock_guard guard(SessionsMutex);
    auto it = Sessions.find(sessionId);
    if (it != Sessions.end()) {
        return it->second;
...
void TYdbMeta::UpdateCluster(const TString& name, const NJson::TJsonValue& cluster) {
    std::lock_guard guard(ClusterInfoCacheMutex);
    auto it = ClusterInfoCache.find(name);
    if (it != ClusterInfoCache.end()) {
        it->second.Cluster = cluster;
        it->second.UpdatedAt = TInstant::Now();
    } else {
        ClusterInfoCache[name] = {cluster, TInstant::Now()};
    }
}

NJson::TJsonValue TYdbMeta::GetCluster(const TString& name) {
    std::lock_guard guard(ClusterInfoCacheMutex);
    auto it = ClusterInfoCache.find(name);
    if (it != ClusterInfoCache.end()) {
        if (it->second.UpdatedAt + TDuration::Seconds(60) > TInstant::Now()) {
    ...
    if (!yamlConfigPath.empty()) {
        try {
            ...
            if (meta) {
                ...
                MetaClusterName = meta["meta_cluster_name"].as<std::string>("");
            }

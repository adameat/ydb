...
void NMeta::JoinJsonValues(NJson::TJsonValue& output, NJson::TJsonValue input) {
    if (!input.IsDefined()) {
        return;
    }
    if (!output.IsDefined()) {
...
void NMeta::JoinJsonValuesToArray(NJson::TJsonValue& output, NJson::TJsonValue input) {
    if (!output.IsDefined()) {
        if (input.IsArray()) {
            output = std::move(input);
        } else {
...
bool NMeta::Compare(const NJson::TJsonValue& a, const NJson::TJsonValue& b) {
    switch (a.GetType()) {
...
public:
    ...
    void OnMapDone(const NActors::TActorContext& ctx) {
        --MapsToGo;
        BLOG_D("Map done (step " << Step + 1 << "/" << Rules.size() << ", " << MapsToGo << " maps to go)");
        if (MapsToGo == 0) {
            if (FinalReducer) {
    ...
    void Handle(TEvJsonMerger::TEvJsonParseResult::TPtr event, const NActors::TActorContext& ctx) {
        ...
        BLOG_D("Parse done " << peerState.ParseTime.MilliSeconds() << "ms for " << peer.GetName());
    ...
    void Handle(TEvJsonMerger::TEvJsonMapResult::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (itPeerState != ContextPeer.end()) {
            ...
            BLOG_D("Map done " << peerState.MapTime.MilliSeconds() << "ms for " << peer.GetName());
            if (!event->Get()->Error.empty()) {
                Error.emplace_back(event->Get()->Error);
                peerState.Error = event->Get()->Error;
                OnMapDone(ctx);
    ...
    void Handle(TEvJsonMerger::TEvJsonReduceResult::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (itPeerState != ContextPeer.end()) {
            ...
            BLOG_D("Reducer done " << peerState.ReduceTime.MilliSeconds() << "ms for " << peer.GetName());
            if (!event->Get()->Error.empty()) {
                Error.emplace_back(event->Get()->Error);
                peerState.Error = event->Get()->Error;
                OnMapDone(ctx);
            ...
            BLOG_D("Reducer done (step " << Step + 1 << "/" << Rules.size() << ")");
            TJsonMergeContextPtr filterContext = event->Get()->Context; // we reuse reducer context for filter
            NActors::TActorId filter = CreateJsonFilter(filterContext, ctx);
            ctx.Send(filter, new TEvJsonMerger::TEvJsonFilter(std::move(event->Get()->JsonValue)));
            ++Requests;
    ...
    void Handle(TEvJsonMerger::TEvJsonFilterResult::TPtr event, const NActors::TActorContext& ctx) {
        ...
        if (itPeerState != ContextPeer.end()) {
            ...
            BLOG_D("Filter done " << peerState.FilterTime.MilliSeconds() << "ms for " << peer.GetName());
            if (!event->Get()->Error.empty()) {
                Error.emplace_back(event->Get()->Error);
                peerState.Error = event->Get()->Error;
                OnMapDone(ctx);
            ...
            BLOG_D("Filter done (step " << Step + 1 << "/" << Rules.size() << ")");
            ++Step;
            if (Step < Rules.size()) {
                MapsToGo = 1;
                TJsonMergeContextPtr reduceContext = new TJsonMergeContext(Rules[Step], FinalReduceOperationState);
        ...
        if (Response == nullptr) {
            if (!Error.empty()) {
                ...
                if (Result.IsMap() && Result.Has("_http")) {
                    ...
                    BLOG_D("Renderer lag " << (TInstant::Now() - now).MilliSeconds() << "ms");
                }
            }
            ctx.Send(IncomingConnectionId, new NHttp::TEvHttpProxy::TEvHttpOutgoingResponse(Response));
            BLOG_D("Response size " << Response->Body.size()
...
TErrorHandler NMeta::IgnoreAllErrors() {
...
NActors::TActorId NMeta::CreateJsonMerger(const NActors::TActorId& httpProxyId,
                                         const NActors::TActorId& incomingConnectionId,
                                         NHttp::THttpIncomingRequestPtr request,

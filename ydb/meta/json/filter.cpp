...
void NMeta::FilterJsonValues(NJson::TJsonValue& output, TJsonMergeContext& context) {
...
NMeta::TJsonFilter NMeta::FilterArrayToCount() {
...
NMeta::TJsonFilter NMeta::SortArrayOfObjects(const TString& sortKey) {
    char direction = '+';
    TString key = sortKey;
        ...
        if (output.IsArray()) {
            ...
            case '+':
                Sort(output.GetArraySafe(), [&key](const NJson::TJsonValue& a, const NJson::TJsonValue& b) -> bool {
                    return NMeta::Compare(a[key], b[key]);
                });
                break;
            case '-':
                Sort(output.GetArraySafe(), [&key](const NJson::TJsonValue& a, const NJson::TJsonValue& b) -> bool {
                    return NMeta::Compare(b[key], a[key]);
                });
                break;
            }
        } else {
...
NMeta::TJsonFilter NMeta::LimitArraySize(size_t size) {
    if (size == 0) {
        return [](NJson::TJsonValue&, TJsonMergeContext&) {};
    }
...
NMeta::TJsonFilter NMeta::MultipleFilters(const TVector<TJsonFilter>& filters) {
...
NActors::TActorId NMeta::CreateJsonFilter(TJsonMergeContextPtr context, const NActors::TActorContext& ctx) {
